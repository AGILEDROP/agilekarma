...
const Chart = (props: ChartInterface) => {
  ...
  const results = !searchValue
    ? users
    : (users as []).filter((user: { item: string }) =>
    ...
    <>
      {listChannels.length !== 0 ? (
        <DateRange
          ...
          onChannelClick={(value: string) => setChannel(value)}
          onStartDateClick={(value: any) => setStartDate(value)}
          onEndDateClick={(value: string) => setEndDate(value)}
          onSearchClick={(value: string) => props.onClick(value)}
          onFilterClick={(value: number) => setPaginationSearch(0)}
        />
      ) : null}
          ...
          <div className="row">
            <div className="col">
              <div className="card-deck">
                {users
                  ? (users.slice(0, 3) as []).map(
                      (
                        user: { rank: number; item: string; score: number },
                        index: number
                      ) => (
                        <div
                          className={`${
                        ...
                        >
                          ...
                          <h4>{user.item}</h4>
                          <div className="card-footer score">{user.score}</div>
                        </div>
                      )
                    )
                  : null}
            ...
            <div className="row mt-5">
              <div className="col text-center">
                ...
                <p>
                  {listChannels
                    ? listChannels.map(
                        (el: { channel_id: string; channel_name: string }) => {
